...
def calc_fully_connected_NN(x_data, y_data):
    ...
    class neurons(object):
        def __init__(self, shape, bias):
            ...
            self.w = [np.full((n,m),1) for n,m in zip(shape[1:],shape[:-1])]
            self.b = [np.full((n,1),0) for n in shape[1:]]
            self.b[1] = np.array([[bias]])
    ...
    for i in range(len(B)):
        model = neurons([1,8,1,8,1],B[i])
        loss[i] = model.evaluate(x_data, y_data)
    #### END YOUR CODE HERE ####
    return loss
