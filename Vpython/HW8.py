...
# handle collision here
    ...
    collision_or_not = np.less_equal(p_array, prob)
    this_loop_c = np.sum(collision_or_not)

    theta = 2*math.pi*np.random.rand(this_loop_c)
    phi = 2*math.pi*np.random.rand(this_loop_c)

    v_array[collision_or_not] = vrms * np.transpose(np.array([np.cos(theta)*np.cos(phi), np.cos(theta)*np.sin(phi), np.sin(theta)]))
    ...
    if int(t/dt)%2000 == 0:
        tau = t*N/total_c 
        print("tau=",tau," Vd_theoretical=" , q*E*tau/m, " mag(", vv/(t/dt)," )=", mag(vv/(t/dt)))
