...
class neurons(object):
    def __init__(self, shape,W):
        ...
        self.w = [np.full((n,m),W) for n,m in zip(shape[1:],shape[:-1])]
        self.b = [np.linspace(0,0.9,n).reshape((n,1)) for n in shape[1:]]
        self.b[-1] = np.full(shape[-1], 1.).reshape((4,1))
...
def train_NN_gradient_descent(x_train, y_train, W):
    ...
    bias = model.b[0].reshape(10)
    #### END YOUR CODE HERE ####
    return bias

# x_data = np.array([-4.0349994,0.84464265, -2.2847417,-2.94720232])
# y_data = np.array([1.,0.,1.,0.])
# W = 0.1664553486059548 
